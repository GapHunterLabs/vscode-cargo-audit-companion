 ...
 * a real number of cargo audit runs that found real vulnerabilities -- never on install, never on a timer.
...
const KEY_HIT_COUNT = 'cargoAuditCompanion.review.hitCount';
const KEY_ANSWERED = 'cargoAuditCompanion.review.answered';
...
/**
 * Call this once per real cargo audit runs that found real vulnerabilities. Safe to call multiple times; a
...
function showPrompt(context: vscode.ExtensionContext): void {
  ...
  void vscode.window
    .showInformationMessage(
      `Cargo Audit Companion: you've used this ${HITS_BEFORE_PROMPT} times -- if it's saved you time, a rating on the Marketplace helps other developers find it.`,
      rateAction,
      dismissAction,
    )
    .then((selection) => {
